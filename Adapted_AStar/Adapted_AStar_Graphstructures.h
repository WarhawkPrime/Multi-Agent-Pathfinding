...
public:
	...
	void get_adjacencies(int node_id, std::vector<std::pair<int, int>>& adj) { adj = this->adjacency_map.at(node_id); }
	...
	double get_h_costs(int node_id) const { return vertex_map.at(node_id).get()->cost_h; }
	void set_h_cost(int node_id, double new_cost_h) { vertex_map.at(node_id).get()->cost_h = new_cost_h; }

	double get_f_costs(int node_id) const { return vertex_map.at(node_id).get()->cost_f; }
	void set_f_cost(int node_id, double new_cost_f) { vertex_map.at(node_id).get()->cost_f = new_cost_f; }
